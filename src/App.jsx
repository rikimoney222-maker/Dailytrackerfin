...
import {
  Trash2, Plus, Bell, BellOff, CheckCircle2, UtensilsCrossed, BarChart2,
  Settings, ListTodo, Clock, RefreshCw, Flame, Snowflake, Brain,
  Send, AlertCircle, BookOpen, Search, BookMarked, Camera,
  Play, Pause, RotateCcw, Leaf, Wind, Sparkles, Music, Volume2,
} from "lucide-react";
...
/* ── Sound options for yoga music panel ── */
const SOUND_OPTIONS = [
  {
    id: "meditation",
    label: "Медитація",
...
export default function App() {
  ...
  const [tasks, setTasks] = useState(() => {
    try { return JSON.parse(localStorage.getItem("dt_tasks") || "[]"); } catch { return []; }
  });
  ...
  // ── Music state ──
  const [soundType, setSoundType] = useState(null);   // "meditation" | "nature" | "ocean" | null
  ...
  const volumeRef     = useRef(volume); // keep in sync without re-creating audio

  // Keep volumeRef in sync
  useEffect(() => {
    volumeRef.current = volume;
    if (masterGainRef.current && audioCtxRef.current) {
      // Don't touch LFO-modulated param; instead use a separate volume gain
      // masterGainRef actually IS the volume gain; LFO targets a separate node
    }
  }, [volume]);
  ...
  /* ══════════════════════════════════════════════
     ...
     No external URLs needed, works offline.
  ══════════════════════════════════════════════ */

  /** Gracefully stop all audio nodes, optionally with a fade-out */
  const stopAudio = useCallback((fade = false) => {
    const ctx = audioCtxRef.current;
    if (!ctx) return;

    if (fade && masterGainRef.current && ctx.state !== "closed") {
      try {
      ...
      setTimeout(() => {
        ...
        try { capturedCtx.close(); } catch (_) {}
        if (audioCtxRef.current === capturedCtx) {
  ...
  /** Build a stereo white-noise buffer (loopable) */
  const makeNoiseBuffer = (ctx, seconds = 4) => {
    const sr = ctx.sampleRate;
    const buf = ctx.createBuffer(2, sr * seconds, sr);
    for (let ch = 0; ch < 2; ch++) {
      const d = buf.getChannelData(ch);
      // Pink-ish noise: average of several random values
      ...
      for (let i = 0; i < d.length; i++) {
        ...
        d[i] = (b0+b1+b2+b3+b4+white*0.5362) * 0.14;
      }
    }
    return buf;
  };

  /** Start the chosen ambient sound */
  const startAudio = useCallback((type, vol) => {
    ...
    // Master volume gain (user-controlled)
    ...
    if (type === "meditation") {
      /* ── 432 Hz Om drone with 3 harmonics + slow tremolo ── */
      const baseFreqs = [136.1, 272.2, 408.3]; // Om / Solfeggio overtones
      const gains     = [0.55, 0.28, 0.12];

      // Slow volume pulse (breathing rhythm ~6 s)
      const tremoloLfo  = ctx.createOscillator();
      const tremoloGain = ctx.createGain();
      tremoloLfo.frequency.value = 0.16;  // ~6 s cycle
      tremoloGain.gain.value     = 0.18;  // depth of tremolo
      tremoloLfo.connect(tremoloGain);
      tremoloLfo.start();
      nodes.push(tremoloLfo, tremoloGain);
      ...
      baseFreqs.forEach((freq, i) => {
        ...
        tremoloGain.connect(gain.gain);  // LFO modulates each gain
        osc.connect(gain);
      ...
      // Subtle high bell overtone at 720 Hz
      const bell  = ctx.createOscillator();
      const bellG = ctx.createGain();
      bell.type            = "sine";
      bell.frequency.value = 720;
      bellG.gain.value     = 0.04;
      bell.connect(bellG);
      bellG.connect(masterGain);
      bell.start();
      nodes.push(bell, bellG);

    } else if (type === "nature") {
      /* ── Rain: pink noise → bandpass filters ── */
      const noiseBuf = makeNoiseBuffer(ctx, 6);
      const source   = ctx.createBufferSource();
      source.buffer  = noiseBuf;
      source.loop    = true;

      // Two bandpass layers for rain texture
      ...
      bp1.connect(mix1);
      bp2.connect(mix2);
      mix1.connect(masterGain);
      mix2.connect(masterGain);
      source.start();

      // Occasional "drip" using a very brief sine burst
      nodes.push(source, bp1, bp2, mix1, mix2);

    } else if (type === "ocean") {
      /* ── Ocean waves: pink noise → lowpass + slow LFO amplitude ── */
      ...
      // Wave LFO — one "wave" every ~9 s
      const waveLfo  = ctx.createOscillator();
      const waveGain = ctx.createGain();
      waveLfo.type           = "sine";
      waveLfo.frequency.value = 0.11;
      waveGain.gain.value    = 0.35;  // wave swell depth
      waveLfo.connect(waveGain);
      waveLfo.start();

      // Second, faster LFO for ripple texture
      const rippleLfo  = ctx.createOscillator();
      const rippleGain = ctx.createGain();
      rippleLfo.type            = "sine";
      rippleLfo.frequency.value = 0.38;
      rippleGain.gain.value     = 0.1;
      rippleLfo.connect(rippleGain);
      rippleLfo.start();

      // Wave envelope gain
      const waveEnv = ctx.createGain();
      waveEnv.gain.value = 0.55;
      waveGain.connect(waveEnv.gain);
      rippleGain.connect(waveEnv.gain);

      source.connect(lp);
      lp.connect(waveEnv);
      waveEnv.connect(masterGain);
      source.start();

      nodes.push(source, lp, waveLfo, waveGain, rippleLfo, rippleGain, waveEnv);
    }
  ...
  /** Toggle play / pause for the currently selected sound */
  const toggleAudio = useCallback(() => {
    if (!soundType) return;
    if (isPlaying) {
      stopAudio(true);
    } else {
      startAudio(soundType, volumeRef.current);
    }
  }, [soundType, isPlaying, startAudio, stopAudio]);

  /** Select a sound type; auto-start playing it */
  const handleSoundSelect = useCallback((id) => {
    setSoundType(id);
    startAudio(id, volumeRef.current);
  }, [startAudio]);
  ...
  // Fade out music when leaving yoga tab; stop on unmount
  useEffect(() => {
    if (tab !== "yoga") stopAudio(true);
  }, [tab, stopAudio]);
  ...
  const playBell = useCallback(() => {
    try {
      ...
      // Two overlapping sine tones for a clear bell timbre
      [[880, 1], [1320, 0.5], [2200, 0.2]].forEach(([freq, amp], i) => {
        const osc  = ctx.createOscillator();
        const gain = ctx.createGain();
        osc.type           = "sine";
        osc.frequency.value = freq;
        gain.gain.setValueAtTime(amp * 0.6, ctx.currentTime);
  ...
  // ── Task reminder checker (runs every 30 s) ──
  useEffect(() => {
    const check = () => {
      ...
      setTasks((prev) =>
        prev.map((task) => {
          if (
            task.reminderTime === hhmm &&
            !task.completed &&
            task.bellEnabled !== false
          ) {
            const key = `reminder_${task.id}_${today}_${hhmm}`;
            if (!fired.current[key]) {
              fired.current[key] = true;
              new Notification("🔔 DayTracker: Час виконати завдання!", {
                body: task.text,
    ...
    try {
      ...
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 400,
          messages: [
            { role: "system", content: "Ти — шеф-кухар здорового харчування. Відповідай ТІЛЬКИ валідним JSON без будь-якого додаткового тексту, markdown або пояснень." },
            { role: "user", content: `Придумай корисний рецепт на ${mealLabel}. Надай відповідь строго у форматі JSON: { "name": "Назва", "emoji": "🍎", "time": "Х хв", "ing": ["інгредієнт 1", "інгредієнт 2", "інгредієнт 3"], "steps": "опис приготування у 2-3 реченнях" }. Мова: українська. Тільки JSON, без додаткового тексту.` },
          ],
        }),
      });
      const data = await res.json();
  ...
  const toggleBell = (id) => setTasks((p) => p.map((t) => t.id === id ? { ...t, bellEnabled: t.bellEnabled === false ? true : false } : t));
  ...
  const scanBookCover = async (e) => {
    ...
    if (!file) return;
    e.target.value = "";
    if (!groqKey) { showToast("⚠️ Спочатку введи Groq API ключ у Налаштуваннях"); return; }
  ...
  return (
    <div style={s.wrap}>
      ...
      <style>{`
        ...
        @keyframes soundBeat  { 0%,100%{transform:scaleY(1)} 50%{transform:scaleY(1.6)} }
        @keyframes fadeInUp   { from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:translateY(0)} }

        /* Animated equalizer bars */
        .eq-bars span { display:inline-block; width:3px; border-radius:2px; background:#10b981; margin:0 1px; animation:soundBeat .8s ease-in-out infinite; }
        .eq-bars span:nth-child(2){ animation-delay:.15s; animation-duration:.65s; }
        .eq-bars span:nth-child(3){ animation-delay:.3s;  animation-duration:.9s;  }
        .eq-bars span:nth-child(4){ animation-delay:.1s;  animation-duration:.7s;  }

        /* Volume slider */
        input[type=range].vol-slider { -webkit-appearance:none; appearance:none; height:4px; border-radius:99px; outline:none; cursor:pointer; }
        input[type=range].vol-slider::-webkit-slider-thumb { -webkit-appearance:none; width:14px; height:14px; border-radius:50%; background:#10b981; cursor:pointer; box-shadow:0 0 6px #10b98166; }
        input[type=range].vol-slider::-moz-range-thumb { width:14px; height:14px; border-radius:50%; background:#10b981; cursor:pointer; border:none; }
      `}</style>
      ...
      <header style={s.header}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {isPlaying && tab === "yoga" && (
            <div style={{ display: "flex", alignItems: "center", gap: 5, backgroundColor: "#052e16", padding: "3px 10px", borderRadius: 8, border: "1px solid #10b98133" }}>
              <div className="eq-bars" style={{ display: "flex", alignItems: "flex-end", height: 14 }}>
                <span style={{ height: 8 }} /><span style={{ height: 12 }} /><span style={{ height: 6 }} /><span style={{ height: 10 }} />
              </div>
              <span style={{ fontSize: 11, color: "#10b981", fontWeight: 600 }}>{SOUND_OPTIONS.find(o => o.id === soundType)?.emoji}</span>
            </div>
          )}
      ...
      <main style={{ maxWidth: 520, margin: "0 auto", padding: "16px 14px 90px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  ...
                  style={{ flex: 1, backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: 14, padding: "13px 16px", color: "#fff", fontSize: 15, outline: "none" }}
                />
                <button onClick={addTask} style={{ backgroundColor: "#7c3aed", border: "none", borderRadius: 14, padding: "13px 18px", cursor: "pointer", color: "#fff", display: "flex", alignItems: "center" }}>
                  <Plus size={22} />
                </button>
              </div>
              {/* Time reminder row */}
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 4px" }}>
                <Bell size={13} color={inputTime ? "#a78bfa" : "#334155"} style={{ flexShrink: 0 }} />
                <span style={{ fontSize: 12, color: "#475569" }}>Нагадати о:</span>
                <input
                  type="time"
                  value={inputTime}
                  ...
                  style={{
                    ...
                    border: `1px solid ${inputTime ? "#7c3aed" : "#334155"}`,
                    borderRadius: 8,
                    padding: "5px 10px",
                    color: inputTime ? "#a78bfa" : "#64748b",
                    fontSize: 13,
                    outline: "none",
                    cursor: "pointer",
                  }}
                ...
                {inputTime && (
                  <button
                    onClick={() => setInputTime("")}
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#475569", fontSize: 12, padding: "0 4px" }}
                  >✕</button>
                )}
              </div>
            </div>
            ...
            {tasks.length === 0 ? (
              ...
              <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 8 }}>
                {sortedTasks.map((task) => {
                  const bellOn = task.bellEnabled !== false;
                  return (
                    <li key={task.id} style={{
                      backgroundColor: task.isFrog ? "#0d0a1a" : "#0f172a",
                      border: task.isFrog ? "1.5px solid #7c3aed" : "1px solid #1e293b",
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ flex: 1, fontSize: 15, color: task.completed ? "#475569" : "#e2e8f0", textDecoration: task.completed ? "line-through" : "none" }}>
                          {task.text}
                        </span>
                        {/* Bell toggle */}
                        <button
                          onClick={() => toggleBell(task.id)}
                          title={bellOn ? "Вимкнути нагадування" : "Увімкнути нагадування"}
                          style={{ background: "none", border: "none", cursor: "pointer", padding: 3, display: "flex", color: bellOn && task.reminderTime ? "#a78bfa" : "#334155", flexShrink: 0 }}
                        >
                          {bellOn ? <Bell size={15} /> : <BellOff size={15} />}
                        </button>
                        {/* Frog toggle */}
                        ...
                        {/* Delete */}
                        <button onClick={() => deleteTask(task.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#334155", padding: 3, display: "flex" }} onMouseEnter={(e) => (e.currentTarget.style.color = "#f87171")} onMouseLeave={(e) => (e.currentTarget.style.color = "#334155")}>
                          <Trash2 size={16} />
                        </button>
                      </div>
                      {/* Reminder badge */}
                      {task.reminderTime && (
                        <div style={{ display: "flex", alignItems: "center", gap: 5, paddingLeft: 28 }}>
                          <div style={{
                            ...
                            backgroundColor: bellOn ? "#1e1035" : "#1a1a2e",
                            border: `1px solid ${bellOn ? "#7c3aed55" : "#334155"}`,
                            borderRadius: 6, padding: "2px 8px",
                          }}>
                            <Clock size={10} color={bellOn ? "#a78bfa" : "#475569"} />
                            <span style={{ fontSize: 11, color: bellOn ? "#a78bfa" : "#475569", fontWeight: 600 }}>
                              {task.reminderTime}
                            </span>
                            {!bellOn && <span style={{ fontSize: 10, color: "#475569" }}>· вимкнено</span>}
                          </div>
                        </div>
                      )}
                    </li>
            ...
            {Object.entries(MEAL_META).map(([meal, meta]) => {
              ...
              return (
                <div key={meal} style={s.card}>
                  ...
                  <button onClick={() => generateRecipe(meal)} disabled={isLoading} style={{ width: "100%", border: isLoading ? "1px solid #7c3aed44" : "1px solid #334155", borderRadius: 10, padding: "10px 0", color: isLoading ? "#a78bfa" : "#94a3b8", fontSize: 13, fontWeight: 600, cursor: isLoading ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "all .2s", background: isLoading ? "linear-gradient(90deg, #1e293b 25%, #2d1f6e 50%, #1e293b 75%)" : "#1e293b", backgroundSize: isLoading ? "200% auto" : "auto", animation: isLoading ? "shimmer 1.5s linear infinite" : "none" }}>
                    <RefreshCw size={13} style={{ animation: isLoading ? "spin 1s linear infinite" : "none" }} />
                    {isLoading ? "Шеф готує..." : rec ? "Інший рецепт від AI" : hasKey ? "✨ Отримати AI-рецепт" : "Отримати рецепт (потрібен API ключ)"}
                  </button>
                  {!hasKey && !rec && <p style={{ margin: "8px 0 0", fontSize: 11, color: "#475569", textAlign: "center" }}>⚙️ Додай Groq API ключ у Налаштуваннях, щоб генерувати рецепти</p>}
                  {rec && !isLoading && (
                    <div style={{ marginTop: 12, backgroundColor: "#020617", borderRadius: 10, padding: 14 }}>
                      ...
                      <p style={{ margin: "0 0 10px", fontSize: 12, color: "#94a3b8" }}>{Array.isArray(rec.ing) ? rec.ing.join(" • ") : rec.ing}</p>
        ...
        {tab === "yoga" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{
              ...s.card,
              border: isPlaying ? "1px solid #10b98166" : "1px solid #1e293b",
              background: isPlaying
                ? "linear-gradient(160deg,#030f09 60%,#0a1f14)"
                : "#0f172a",
              transition: "all .4s",
              animation: isPlaying ? "fadeInUp .3s ease" : "none",
            }}>
              {/* Header row */}
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <Music size={17} color={isPlaying ? "#10b981" : "#475569"} />
                  <span style={{ fontWeight: 700, fontSize: 15, color: isPlaying ? "#fff" : "#94a3b8" }}>
                    Музичний супровід
                  </span>
                </div>
                {isPlaying && (
                  <div style={{ display: "flex", alignItems: "center", gap: 6, backgroundColor: "#052e16", border: "1px solid #10b98133", padding: "3px 10px", borderRadius: 99 }}>
                    <div className="eq-bars" style={{ display: "flex", alignItems: "flex-end", height: 12, gap: 1 }}>
                      <span style={{ height: 6 }} /><span style={{ height: 10 }} /><span style={{ height: 5 }} /><span style={{ height: 8 }} />
                    </div>
                    <span style={{ fontSize: 11, color: "#10b981", fontWeight: 600 }}>грає</span>
                  </div>
                )}
              </div>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                {SOUND_OPTIONS.map((opt) => {
                  const active = soundType === opt.id && isPlaying;
                  const selected = soundType === opt.id;
                  return (
                    <button
                      key={opt.id}
                      ...
                      style={{
                        ...
                        borderRadius: 12,
                        border: `1.5px solid ${active ? opt.border : selected ? opt.border + "77" : "#1e293b"}`,
                        backgroundColor: active ? opt.bg : selected ? opt.bg + "88" : "#1e293b",
                        color: active ? opt.color : selected ? opt.color + "bb" : "#475569",
                        fontSize: 11,
                        fontWeight: 700,
                        cursor: "pointer",
                        display: "flex",
                        ...
                        gap: 5,
                        transition: "all .25s",
                        boxShadow: active ? `0 0 16px ${opt.border}44` : "none",
                      }}
                    >
                      <span style={{ fontSize: 20, lineHeight: 1 }}>{opt.emoji}</span>
                      <span>{opt.label}</span>
                      <span style={{ fontSize: 10, opacity: 0.7, fontWeight: 500 }}>{opt.desc}</span>
                    </button>
                  );
                })}
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, padding: "0 2px" }}>
                <Volume2 size={14} color="#475569" style={{ flexShrink: 0 }} />
                <input
                  type="range"
                  ...
                  style={{
                    flex: 1,
                    background: `linear-gradient(to right, #10b981 ${volume * 100}%, #1e293b ${volume * 100}%)`,
                  }}
                />
                <span style={{ fontSize: 11, color: "#475569", minWidth: 28, textAlign: "right" }}>
              ...
              <button
                ...
                style={{
                  ...
                  borderRadius: 12,
                  padding: "13px 0",
                  background: !soundType
                    ? "#1e293b"
                    : isPlaying
                    ? "linear-gradient(135deg,#065f46,#10b981)"
                    : "linear-gradient(135deg,#064e3b,#059669)",
                  color: !soundType ? "#334155" : "#fff",
                  fontWeight: 700,
                  fontSize: 14,
                  cursor: !soundType ? "not-allowed" : "pointer",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  gap: 8,
                  transition: "all .25s",
                  boxShadow: isPlaying ? "0 0 20px #10b98144" : "none",
                }}
              >
                {isPlaying
                  ? <><Pause size={16} /> Зупинити</>
                  : soundType
                  ? <><Play size={16} style={{ marginLeft: 2 }} /> Увімкнути звук</>
                  : "Оберіть тип звуку вище"
                }
              </button>
              ...
              {!soundType && (
                <p style={{ margin: "10px 0 0", fontSize: 11, color: "#334155", textAlign: "center" }}>
                  Звуки генеруються прямо в браузері — без інтернету 🎧
                </p>
              )}
              {isPlaying && (
                <p style={{ margin: "10px 0 0", fontSize: 11, color: "#10b98177", textAlign: "center" }}>
                  Музика плавно зупиниться при виході з вкладки
                </p>
              )}
            </div>
            ...
            <div style={{ ...s.card, border: "1px solid #10b98133" }}>
              ...
              {!yogaPractice && !yogaLoadingPractice && (
                <p style={{ margin: "10px 0 0", fontSize: 12, color: "#475569", textAlign: "center" }}>
                  {(!groqKey) ? "⚙️ Потрібен Groq API ключ у Налаштуваннях" : "Натисни кнопку — AI підбере 3 пози спеціально для тебе"}
                </p>
              )}
            </div>
            ...
            <div style={s.card}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                <input
                  ...
                  value={groqKey}
                  onChange={(e) => { setGroqKey(e.target.value); setKeySaved(false); }}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      localStorage.setItem("groq_api_key", groqKey);
                      setKeySaved(true);
                  ...
                  placeholder="gsk_..."
                  style={{ ...s.input, fontFamily: "monospace", fontSize: 12, flex: 1 }}
                />
                <button onClick={() => setShowKey((p) => !p)} style={{ backgroundColor: "#1e293b", border: "1px solid #334155", borderRadius: 10, padding: "0 13px", color: "#94a3b8", cursor: "pointer", fontSize: 12, flexShrink: 0 }}>
              ...
              <button
                ...
                style={{
                  ...
                  gap: 8,
                  transition: "all .3s",
                  boxShadow: keySaved ? "0 0 16px #16a34a55" : "none",
                }}
              >
                <CheckCircle2 size={16} />
                {keySaved ? "Збережено!" : "Зберегти ключ"}
              </button>
              {groqKey ? (
                <div style={{ backgroundColor: "#052e16", border: "1px solid #16a34a", borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <p style={{ margin: 0, fontSize: 13, color: "#86efac" }}>Ключ активний. Всі AI-функції увімкнено!</p>
                </div>
              ) : (
