...
export default function App() {
  ...
  // Request notification permission on first load + register SW + unlock iOS audio
  useEffect(() => {
    // 1. Register Service Worker (required for push on iOS PWA)
    ...
    // 3. iOS audio unlock — play a silent buffer on the very first user tap
    //    so that subsequent Web Audio calls (bell sound) are not blocked.
    const unlock = () => {
            ...
            messages: [
              {
                role: "system",
                content:
                  "Ти — інструктор йоги. Відповідай ТІЛЬКИ українською мовою. Давай стислі, практичні рекомендації.",
              },
              {
                role: "user",
                content:
                  "Дай мені рівно 3 пози або поради для сьогоднішньої йога-практики. Формат: кожна порада — окремий рядок, починається з емодзі, максимум 20 слів. Без вступу і висновків.",
              },
            ],
          }),
        }
  ...
  const generateRecipe = async (mealKey) => {
    ...
    setMeals((prev) => ({
      ...prev,
      [mealKey]: { ...prev[mealKey], recipe: null },
    }));
    try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "Ти — шеф-кухар здорового харчування. Відповідай ТІЛЬКИ валідним JSON без будь-якого додаткового тексту, markdown або пояснень.",
              },
              {
                role: "user",
  ...
  const toggleBell = (id) =>
    setTasks((p) =>
      p.map((t) =>
        t.id === id
          ? { ...t, bellEnabled: t.bellEnabled === false ? true : false }
          : t
      )
    );
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {isPlaying && tab === "yoga" && (
            ...
            >
              <div
                className="eq-bars"
                style={{ display: "flex", alignItems: "flex-end", height: 14 }}
              >
                <span style={{ height: 8 }} />
              ...
              <span style={{ fontSize: 11, color: "#10b981", fontWeight: 600 }}>
                {SOUND_OPTIONS.find((o) => o.id === soundType)?.emoji}
              </span>
            </div>
          )}
        ...
        {TABS.map(({ id, label, Icon }) => (
          <button
            ...
            style={{
              ...
              borderBottom:
                tab === id
                  ? `2px solid ${id === "yoga" ? "#10b981" : "#a78bfa"}`
      ...
      >
              ...
              return (
                <div key={meal} style={s.card}>
                  ...
                  {!hasKey && !rec && (
                    ...
                    >
                      ⚙️ Додай Groq API ключ у Налаштуваннях, щоб генерувати
                      рецепти
                    </p>
                  )}
                  {rec && !isLoading && (
        ...
        {tab === "yoga" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            >
              <h2 style={{ ...s.h2, color: "#10b981", margin: 0 }}>
                🧘 Йога & Дихання
              </h2>
              {yogaTotalTime > 0 && (
                <span
            ...
            <div
              style={{
                ...s.card,
                border: "1px solid #10b98133",
                background: "linear-gradient(160deg,#030f09,#0f172a)",
                textAlign: "center",
                padding: "28px 16px",
              }}
            >
              ...
              >
                <div
                  style={{
                    ...
                    animation: `${
                      breathPhase === "inhale" ? "breatheIn" : "breatheOut"
                ...
                >
                  ...
                  <circle
                    ...
                    r="80"
                    fill="none"
                    stroke={yogaRemainSec === 0 ? "#34d399" : "#10b981"}
                    strokeWidth="6"
                    strokeLinecap="round"
                ...
                <div style={{ position: "absolute", textAlign: "center" }}>
                  <p
                    style={{
                      ...
                      fontSize: 36,
                      fontWeight: 800,
                      color: yogaRemainSec === 0 ? "#34d399" : "#fff",
                      letterSpacing: -1,
                    }}
                  >
                    {yogaRemainSec === 0 ? "🎉" : fmtTime(yogaRemainSec)}
                  </p>
                  <p
                    style={{
                      margin: "4px 0 0",
              ...
              <p style={{ margin: "0 0 16px", fontSize: 13, color: "#6b7280" }}>
                {yogaRunning
                  ? "Практика триває · синхронізуй дихання з колом"
                  : "Обери тривалість та натисни старт"}
              </p>
              <div
                style={{
                  display: "flex",
              ...
              >
                {TIMER_PRESETS.map((min) => (
                  <button
                    key={min}
                    onClick={() => selectYogaPreset(min)}
                    disabled={yogaRunning}
                    style={{
                      padding: "8px 18px",
                      borderRadius: 99,
                      fontSize: 13,
                      fontWeight: 600,
                      cursor: yogaRunning ? "not-allowed" : "pointer",
                      border: `1.5px solid ${
              ...
              >
                ...
                <button
                  onClick={toggleYogaTimer}
                  disabled={yogaRemainSec === 0}
                  style={{
                    width: 64,
                    height: 64,
                    borderRadius: "50%",
                    border: "none",
                    cursor: yogaRemainSec === 0 ? "not-allowed" : "pointer",
                    background:
                      yogaRemainSec === 0
                        ? "#1e293b"
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    boxShadow: yogaRunning ? "0 0 24px #10b98166" : "none",
                ...
                {SOUND_OPTIONS.map((opt) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span>{opt.label}</span>
                      <span
                        style={{ fontSize: 10, opacity: 0.7, fontWeight: 500 }}
                      >
                        {opt.desc}
                      </span>
                    </button>
            ...
            <div style={{ ...s.card, border: "1px solid #10b98133" }}>
              ...
              >
                ...
                <span style={{ fontWeight: 700, fontSize: 15, color: "#fff" }}>
                  AI-практика на сьогодні
                </span>
              </div>
              <button
                onClick={generateYogaPractice}
                disabled={yogaLoadingPractice}
                style={{
                  width: "100%",
                  border: "none",
                  borderRadius: 12,
                  padding: "13px 0",
                  background: yogaLoadingPractice
                    ? "#1e293b"
                    : "linear-gradient(135deg,#059669,#10b981)",
                  color: yogaLoadingPractice ? "#64748b" : "#fff",
                  fontWeight: 700,
                  fontSize: 14,
                  cursor: yogaLoadingPractice ? "not-allowed" : "pointer",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  gap: 8,
                  marginBottom: yogaPractice ? 14 : 0,
                }}
              >
                {yogaLoadingPractice ? (
                  <>
                  ...
                  <>
                    <Sparkles size={16} /> Згенерувати практику
                  </>
                )}
              </button>
              {yogaPractice && (
                ...
                >
                  ...
                  >
                    Натисни ще раз, щоб отримати нову практику ✨
                  </p>
                </div>
              )}
              {!yogaPractice && !yogaLoadingPractice && (
                ...
                >
                  {!groqKey
                    ? "⚙️ Потрібен Groq API ключ у Налаштуваннях"
                    : "Натисни кнопку — AI підбере 3 пози спеціально для тебе"}
                </p>
              )}
            </div>
            ...
            >
              ...
              >
                Вдихай <strong style={{ color: "#d1fae5" }}>4 сек</strong> →
                Затримай <strong style={{ color: "#d1fae5" }}>4 сек</strong> →
                Видихай <strong style={{ color: "#d1fae5" }}>4 сек</strong>.
                Коло вгорі допомагає синхронізувати ритм дихання з таймером.
              </p>
            </div>
          </div>
        )}
        ...
        {tab === "settings" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={s.card}>
              ...
              <button
                onClick={() => {
                  if (Notification.permission !== "granted") {
                    showToast("⚠️ Спочатку увімкни сповіщення вище");
                    return;
                  }
                  try {
                    new Notification("🔔 Тест DayTracker", {
              ...
              >
                <Bell size={14} /> Перевірити сповіщення
              </button>
              ...
              >
                ...
                >
                  Для сповіщень на iOS: відкрий сайт у Safari → натисни{" "}
                  <strong style={{ color: "#94a3b8" }}>Поділитися</strong> →{" "}
                  <strong style={{ color: "#94a3b8" }}>
                    На екран «Додому»
                  </strong>
                  . Потім відкривай додаток через іконку на робочому столі.
                </p>
              </div>
            </div>
            <div style={s.card}>
              ...
              {groqKey ? (
                ...
                >
                  ...
                  <p style={{ margin: 0, fontSize: 13, color: "#86efac" }}>
                    Ключ активний. Всі AI-функції увімкнено!
                  </p>
                </div>
              ) : (
                <div
                ...
                >
                  ...
                  <p style={{ margin: 0, fontSize: 13, color: "#fca5a5" }}>
                    Введи ключ для Ментора, сканера книг, рецептів та
                    йога-практик
                  </p>
                </div>
              )}
